...
def G(z, w=g_weights):
    ...
    h2 = tf.sigmoid(tf.matmul(h1, w['out']) + w['b2']) * 255.0
...
def D(x, w=d_weights):
    ...
    # Use h2 to calculate logits loss
...
D_obj_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=dout_real, labels=(tf.ones_like(dout_real)-0.1)))
...
with tf.Session() as sess:
    ...
    # You may wish to save or plot the image generated to see how it looks like
    image = sess.run(G(z), feed_dict={z:generate_z()})
